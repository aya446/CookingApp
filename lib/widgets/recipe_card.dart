...
import 'package:cooking_app/screens/one_recipe_view.dart';
...
class RecipesCard extends StatelessWidget {
    ...
    return FutureBuilder(
      ...
      builder: (context, snapshot) {
        if (snapshot.connectionState == ConnectionState.waiting) {
          ...
          var image = getRandomImage(recipe.category);
          return GestureDetector(
            onTap: () {
              Navigator.of(context).push(
                MaterialPageRoute(
                  builder: (context) {
                    return OneRecipeView(
                      image: image ,
                      recipe: recipe,
                    );
                  },
  ...
  String getRandomImage(String category) {
  final images = ImagesByCategory[category];
  if (images != null && images.isNotEmpty) {
    final random = Random();
    return images[random.nextInt(images.length)];
  }
  return 'assets/images/salads/Frame 66.png'; 
}
}
